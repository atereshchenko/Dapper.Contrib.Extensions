...
namespace Dapper.Contrib.Extensions
{
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        {
            SqlMapper.ITypeMap mapper = (SqlMapper.ITypeMap)Activator.CreateInstance(typeof(ColumnAttributeTypeMapper<>).MakeGenericType(typeof(T)));
        ...
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            var columnAttribute = key?.GetCustomAttributes(typeof(ColumnAttribute), false).FirstOrDefault() as ColumnAttribute;

            string columnName = !string.IsNullOrEmpty(columnAttribute?.Name) ? columnAttribute?.Name : key.Name;

            if (!GetQueries.TryGetValue(type.TypeHandle, out string sql))
            {               
                sql = $"select * from {name} where {columnName} = @{key.Name}";
...
public partial interface ISqlAdapter
{
    ...
    void AppendColumnNameEqualsValue(StringBuilder sb, string AtributColumnName, string ColumnName);
}
...
public partial class SqlServerAdapter : ISqlAdapter
{
    ...
    public void AppendColumnNameEqualsValue(StringBuilder sb, string AtributColumnName, string ColumnName)
    {
        sb.AppendFormat("[{0}] = @{1}", AtributColumnName, ColumnName);
    }
}
